### Fait
- [X] Fetchers RSS + Github fonctionnels
- [X] Appel et condition dans main.py
- [X] Création FastAPI
- [X] Structuration dashboard.yaml
- [X] Utiliser la clé nb_items dans les fetchers python pour requêter le nombre voulu (éventuellement mettre un nb max)
### En cours
- [ ] Création des élements selon les données, script JS
### A faire 
- [ ] Stylisé la page (rouge et lien visible)
- [ ] Permettre de selectionner un article et d'ajouter une page "Favoris" avec une étoile
- [ ] Pour beaucoup plus tard, permettre le suivi de projets
- [ ] Permettre un management de la page web à la main
- [ ] Ajouter des entrées links + tools 
- [ ] Commenter le code
- [ ] Ecrire le readme
- [ ] Permettre le déploiement simplifier de CyberHome